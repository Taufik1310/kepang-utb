...
import { GetChairmanAndViceChairman } from "@/lib/management";
import { motion } from "motion/react";
import Image from "next/image";

const CHAIRMAN = GetChairmanAndViceChairman();

export default function ManagementTopmanChairman() {
  return (
    <div className="flex w-full flex-col gap-8 lg:gap-4">
      {CHAIRMAN.map((item, index) => (
        ...
        >
          <div
            className={`relative aspect-square w-full overflow-hidden border-b border-zinc-50 md:w-4/12 md:border-b-0 ${(index + 1) % 2 == 0 ? "md:order-last md:border-l" : "md:border-r"}`}
          >
            <Image
              src={item.image}
              alt={item.title}
